...
class RouteUpdateSerializer(serializers.Serializer):
    route_name = serializers.CharField(max_length=120, required=False)
    start_point = serializers.CharField(max_length=255, required=False)
    end_point = serializers.CharField(max_length=255, required=False)
    shops = RouteShopInputSerializer(many=True, required=False)
